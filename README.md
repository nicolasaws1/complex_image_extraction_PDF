...
Este script faz parte do pipeline de ingestão do projeto SB100 Agrônomo Virtual. Seu objetivo é extrair todas as figuras dos artigos científicos agronômicos já vetorizados, classificá-las automaticamente por tipo visual, e gerar um dataset estruturado para ser usado pela equipe de Machine Learning no treinamento de um classificador de imagens especializado.

O dataset gerado é a base para que a equipe de ML possa:
- Revisar e corrigir as classificações automáticas
- Treinar um modelo fine-tuned (ex: EfficientNet-B0 ou ResNet-50) especializado em figuras de artigos agronômicos
- Substituir o classificador CLIP atual por um modelo específico do domínio SB100
...
| Squad ML | Treinamento do classificador | Consome o dataset gerado aqui |
...
PDFs armazenam imagens como objetos binários (xref). Extrair por xref retorna fragmentos — logos de revista, watermarks, barras isoladas de gráfico armazenadas como objetos separados. O classificador recebia esses fragmentos sem contexto e classificava incorretamente.
...
### 3.3 Filtro de página — 3 camadas (`pagina_tem_figura`)

Idêntico ao filtro usado no Script 1 do pipeline principal. Descartou **552 páginas** de texto puro na última execução.

- **Camada 1 — Cor HSV:** páginas com >1.5% de pixels coloridos contêm figuras científicas
- **Camada 2 — Fill density:** blobs P&B com densidade de preenchimento >13% são candidatos
- **Camada 3 — Direção de bordas Sobel:** ratio H/V >2.0 indica texto; distribuído indica figura

### 3.4 Filtro de crop individual — 4 camadas (`crop_e_figura`)

Aplicado em cada recorte antes do CLIP. Descartou **1.277 recortes** de texto na última execução.

- **Camada 1 — Cor:** >1.2% de cor no crop → figura, passa direto
- **Camada 2 — Proporção:** ratio largura/altura >5.0 → parágrafo de texto, descarta
- **Camada 3 — Linhas horizontais regulares (HoughLinesP):** >10 linhas horizontais → texto corrido, descarta
- **Camada 4 — Bordas Sobel no crop:** ratio H/V >2.5 → texto, descarta

### 3.5 Extração de caption

Para cada figura, o script busca no texto da página um bloco que esteja espacialmente próximo (≤45pt, ~1.5cm) à figura e que comece com palavra-chave de legenda em português, inglês ou espanhol (`Figura`, `Fig.`, `Gráfico`, `Figure`, `Chart`, `Tabla`, etc.).

A caption é salva na coluna `caption` do `metadata.csv`. Figuras sem legenda detectável têm caption vazio.
...
│   ├── baixa_confianca/  ← CLIP com confiança < 22%
...
| `confianca` | float | Confiança do CLIP (0.0 a 1.0). 0.0 para rejeitadas por filtro |
...
| Páginas descartadas (só texto) | 552 |
| Crops descartados (filtro texto) | 1.277 |
| Imagens classificadas | 415 |
| Imagens com caption extraída | 23 / 1.693 (1.3%) |
| Rejeitadas — tamanho | 0 |
| Rejeitadas — baixa confiança | 1 |
...
| `tabela_imagem` | 58 | Pode conter ruído — tabelas de texto renderizadas |
| `icone` | 70 | Suspeito — pode incluir logos coloridos de revista |
...
| `diagrama` | 17 | Qualidade média — confunde com fluxograma |
| `fluxograma` | 4 | Baixo — filtro de bordas descarta alguns casos |
| `grafico_pizza` | 0 | Não detectado — CLIP tem dificuldade com este domínio |
...
O modelo CLIP foi treinado em imagens gerais da internet, não em figuras de artigos científicos agronômicos. As classificações são uma **aproximação inicial, não ground truth**. Categorias ambíguas (`diagrama` vs. `fluxograma`, `icone` vs. `outro`) têm qualidade inferior.

### 8.2 Extração de caption

Apenas 1.3% das imagens têm caption extraída. O motivo é que a maioria das legendas nos PDFs do corpus não está espacialmente adjacente à figura no arquivo digital — o layout de duas colunas e o fluxo de texto dos artigos científicos frequentemente posiciona a legenda fora da margem de busca de 45pt.
...
## 9. Instruções para a Equipe de ML
...
- Toda a pasta `outro/` — 135 imagens, principal fonte de ruído
- Amostra de 20% da pasta `icone/` — pode conter logos de revista classificados incorretamente
- Toda a pasta `rejeitadas/filtro_texto/` — pode conter falsos negativos do filtro OpenCV
- Toda a pasta `rejeitadas/baixa_confianca/` — imagens que o CLIP não soube classificar

### 9.4 Modelo recomendado para fine-tuning

| Modelo | Vantagem | Quando usar |
|---|---|---|
| EfficientNet-B0 | Leve, rápido, bom com dataset pequeno (~300 exemplos/classe) | Primeira iteração |
| ResNet-50 | Mais robusto, padrão da indústria | Dataset com >500 exemplos/classe |
